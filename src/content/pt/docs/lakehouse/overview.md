...
O [Analytics plugin](/docs/appkit/v0/plugins/analytics) cuida das leituras no SQL warehouse (consulte [Leituras analíticas](/docs/lakehouse/analytical-reads) e [Pipelines e atualidade dos dados](/docs/lakehouse/pipelines)). O [Jobs plugin](/docs/appkit/v0/plugins/jobs) cuida do disparo de execuções e do acompanhamento do progresso (consulte [Lakeflow Jobs](/docs/lakehouse/jobs)).
...
- **Leituras com latência de milissegundos em uma requisição do usuário**, como typeahead ou autocompletar. Use o [Lakebase Postgres](/docs/lakebase/overview) diretamente ou replique uma tabela do UC no Lakebase como uma synced table.
- **Gravações transacionais a partir do seu app** (pedidos, sessões, logs de auditoria). Use o [Lakebase Postgres](/docs/lakebase/overview).
- **Perguntas e respostas em linguagem natural sobre tabelas governadas**. Use o [Genie](/docs/agents/genie).
...
| Você quer...                                                          | Template                                                            |
| --------------------------------------------------------------------- | ------------------------------------------------------------------- |
| Replicar uma tabela do UC no Lakebase para leituras de baixa latência | [Sync Tables (Autoscaling)](/templates/sync-tables-autoscaling)     |
| Montar o pipeline completo de UC + Change Data Feed do Lakebase + medalhão | [Operational Data Analytics](/templates/operational-data-analytics) |

## Próximos passos \{#where-to-next\}

- [Leituras analíticas](/docs/lakehouse/analytical-reads) com o plugin Analytics, arquivos SQL e consultas on-behalf-of-user.
- [Lakeflow Jobs](/docs/lakehouse/jobs) para o plugin Jobs, `runNow` e progresso via SSE.
- [Pipelines e atualidade](/docs/lakehouse/pipelines) para sinais de atualidade por meio do plugin Analytics.