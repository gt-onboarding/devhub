...
* **Lakeflow Jobs** para orquestração. Consulte [Lakeflow Jobs](/pt/docs/lakehouse/jobs) para o lado do acionamento pelo app.
...
O nome de arquivo `.obo.sql` executa a consulta como o usuário autenticado. Se o service principal do seu app tiver `SELECT` em `system.lakeflow.pipeline_update_timeline`, remova o `.obo` e a consulta será executada como o app. Consulte [Escrever arquivos SQL](/pt/docs/lakehouse/analytical-reads#author-sql-files) para ver a regra completa de nomenclatura de arquivos.

## Disparar uma atualização a partir do app \{#triggering-a-refresh-from-the-app\}

Não existe um plugin dedicado de Pipelines no AppKit. Chame o SDK diretamente do seu handler com `w.pipelines.startUpdate({ pipelineId })` ou encapsule o pipeline em um [Lakeflow Job](/pt/docs/lakehouse/jobs) e use o Jobs plugin.

## Próximos passos \{#where-to-next\}

Experimente [Medallion Architecture from CDC History Tables](/pt/templates/medallion-architecture-from-cdc) para conhecer o pipeline SDP canônico que gera essas tabelas, ou [Operational Data Analytics](/pt/templates/operational-data-analytics) para o padrão completo de UC + CDC + medalhão.