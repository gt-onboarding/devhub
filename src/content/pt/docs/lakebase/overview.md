...
Esta página traz a visão do AppKit sobre o Lakebase. Para saber mais sobre o Lakebase Postgres em si (projetos, branching, autoscaling, conectividade), consulte a [documentação do Lakebase](https://docs.databricks.com/aws/en/oltp/) ou a agent skill [`databricks-lakebase`](/docs/tools/ai-tools/agent-skills).
...
O plugin cuida automaticamente do refresh do token OAuth e do pool de conexões. Quando implantado, a plataforma injeta os valores de conexão como variáveis de ambiente e o plugin os lê. Não é necessária nenhuma configuração manual. A [referência do plugin `lakebase` do AppKit](/docs/appkit/v0/plugins/lakebase) detalha as opções de configuração do pool e a API completa.
...
- Você está sincronizando dados entre sua carga de trabalho OLTP e o [Data Lakehouse](/docs/lakehouse/overview) via change data capture.
...
[Templates](/templates) são prompts prontos para agentes, organizados por caso de uso. Escolha o que melhor atende às suas necessidades ou consulte o [Guia de início rápido do Lakebase Postgres](/docs/lakebase/quickstart) para ver instruções passo a passo.