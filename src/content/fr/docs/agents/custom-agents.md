...
- **Exécutez-le directement dans votre application** avec le [plugin `agents`](/docs/appkit/v0/plugins/agents). Vous définissez l'agent en code ou en markdown, ou vous exécutez un Supervisor managé via l'adaptateur Supervisor API, sans avoir à déployer d'endpoint distinct. Privilégiez cette approche pour un nouvel agent que vous développez vous-même.
- **Appelez un agent déjà exposé sous forme d'endpoint de serving** avec le [plugin Model Serving](/docs/appkit/v0/plugins/model-serving). Utilisez cette approche pour un Knowledge Assistant, ou pour tout agent déjà déployé en tant qu'endpoint partagé.

## Prérequis \{#prerequisites\}

- Databricks CLI `v1.0.0+` avec un [profil authentifié](/docs/tools/databricks-cli#authenticate).
- Une application AppKit en cours d'exécution. Consultez le [démarrage rapide Apps](/docs/apps/quickstart).
...
Le [plugin `agents`](/docs/appkit/v0/plugins/agents) héberge l&#39;agent dans votre App. Vous le définissez en markdown ou en code, vous y connectez des outils, et il est exposé sur des routes intégrées, sans aucun endpoint à provisionner. Pour un nouvel agent personnalisé ou un Supervisor Agent, commencez ici.
...
Consultez la [référence du plugin `agents`](/docs/appkit/v0/plugins/agents) pour en savoir plus sur les agents markdown, la limitation de portée des outils, les sous-agents et les outils Supervisor hébergés.
...
Pour la mise en œuvre complète, y compris `createApp`, `useServingStream` et les gestionnaires de routes personnalisés, consultez [Appeler un endpoint gouverné depuis AppKit](/docs/agents/ai-gateway#call-a-governed-endpoint-from-appkit).
...
Essayez l'[AI Chat App](/templates/ai-chat-app) pour une configuration complète d'AppKit et d'agent, ou parcourez le [catalogue de modèles](/templates) pour découvrir d'autres approches.