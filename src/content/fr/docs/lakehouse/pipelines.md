...
- **Lakeflow Jobs** pour l'orchestration. Voir [Lakeflow Jobs](/docs/lakehouse/jobs) pour le déclenchement depuis une application.
...
Le suffixe `.obo.sql` exécute la requête sous l&#39;identité de l&#39;utilisateur connecté. Si le service principal de votre application dispose du privilège `SELECT` sur `system.lakeflow.pipeline_update_timeline`, supprimez le `.obo` : la requête s&#39;exécutera alors sous l&#39;identité de l&#39;application. Consultez [Rédiger des fichiers SQL](/docs/lakehouse/analytical-reads#author-sql-files) pour la règle complète de nommage des fichiers.


## Déclencher une actualisation depuis l'application \{#triggering-a-refresh-from-the-app\}

Il n'existe pas de plugin AppKit dédié aux pipelines. Appelez directement le SDK depuis votre handler avec `w.pipelines.startUpdate({ pipelineId })`, ou encapsulez le pipeline dans un [job Lakeflow](/docs/lakehouse/jobs) et utilisez le Jobs plugin.

## Pour aller plus loin \{#where-to-next\}

Consultez [Architecture médaillon à partir de tables d'historique CDC](/templates/medallion-architecture-from-cdc) pour découvrir le pipeline SDP canonique qui produit ces tables, ou [Analytique de données opérationnelles](/templates/operational-data-analytics) pour le modèle de bout en bout UC + CDC + médaillon.