...
* Une application AppKit en cours d&#39;exécution. Voir [Démarrage rapide des Apps](/fr/docs/apps/quickstart).
...
Les autres objets UC relèvent d&#39;autres plugins. Les volumes (stockage de fichiers) passent par le [plugin Files](/fr/docs/appkit/v0/plugins/files). La liste complète des objets UC figure dans [Securable objects](https://docs.databricks.com/aws/en/data-governance/unity-catalog/securable-objects).
...
La ressource correspondante est déclarée dans `databricks.yml`. Consultez [Configuration de l&#39;application](/fr/docs/apps/configuration#resources) pour la liste complète des ressources et les clés `valueFrom`.
...
Pour l&#39;API complète du plugin, y compris les types de paramètres et le streaming Arrow, consultez la [référence du plugin Analytics](/fr/docs/appkit/v0/plugins/analytics).
...
Pour les questions-réponses en langage naturel sur les tables UC (jeux de données curés, base de connaissances et système d&#39;IA composite qui transforme les questions en SQL), utilisez [Genie](/fr/docs/agents/genie). Pour un exemple de configuration fonctionnelle, consultez le modèle [Genie Conversational Analytics](/fr/templates/genie-conversational-analytics). Le plugin Genie figure dans la section Agent Bricks, car il s&#39;agit d&#39;une intégration d&#39;agent et non d&#39;une intégration SQL.

## Et ensuite \{#where-to-next\}

Essayez [Set Up Unity Catalog with External Storage](/fr/templates/unity-catalog-setup) pour provisionner un catalogue, ou [Volume File Manager](/fr/templates/volume-file-upload) pour ajouter des UC Volumes à votre application. Explorez ensuite [Lakeflow Jobs](/fr/docs/lakehouse/jobs) pour déclencher des traitements, ou [Pipelines and freshness](/fr/docs/lakehouse/pipelines) pour les indicateurs de « dernière mise à jour ».