...
埋め込む前にワークスペースで Genie Agent を試すには、[Genie Agent を使用する](https://docs.databricks.com/aws/en/genie-agents/talk-to-genie)を参照してください。コーディングエージェントから作成・管理する場合は、[`databricks-genie-agents`](/docs/tools/ai-tools/agent-skills) エージェントスキルを使用してください。
...
- 稼働中の AppKit アプリ。[Apps クイックスタート](/docs/apps/quickstart)を参照してください。
...
[`genie` プラグイン](/docs/appkit/v0/plugins/genie)を使えば、SSE ストリーミング、認証、会話のリプレイまで面倒を見たうえで、これらすべてをチャット UI に接続できます。
...
`alias` プロパティは、サーバー側の `spaces` 設定のキーと一致している必要があります。`<GenieChat>` は親要素いっぱいに広がるため、高さを固定したコンテナに配置しないと高さが 0 になってしまいます。このコンポーネントは、メッセージの描画、ストリーミングの処理、会話 ID の URL への保存、再読み込み時の履歴の復元を行います。プロパティの全一覧については [GenieChat リファレンス](/docs/appkit/v0/api/appkit-ui/genie/GenieChat) を参照してください。
...
各 ID を `app.yaml` 内の個別のリソースにバインドします。エージェントの切り替え、会話のクリーンアップ、URL 同期に対応した実際に動作する UI については、[Genie Multi-Agent Selector](/templates/genie-multi-space) テンプレートを参照してください。
...
すぐに動く構成一式を試すには [Genie Analytics App](/templates/genie-analytics-app) を、Knowledge Assistant や Supervisor Agent については [カスタムエージェントの endpoint](/docs/agents/custom-agents) を参照してください。