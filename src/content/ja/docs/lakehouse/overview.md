...
[Analytics プラグイン](/docs/appkit/v0/plugins/analytics)はSQL warehouseからの読み取りを担当します（[分析的な読み取り](/docs/lakehouse/analytical-reads)および[パイプラインとデータ鮮度](/docs/lakehouse/pipelines)を参照）。[Jobs プラグイン](/docs/appkit/v0/plugins/jobs)は実行のトリガーと進捗の管理を担当します（[Lakeflow Jobs](/docs/lakehouse/jobs)を参照）。
...
- **ユーザーリクエストに対するサブ秒の読み取り**（先行入力やオートコンプリートなど）。[Lakebase Postgres](/docs/lakebase/overview) を直接使うか、UC テーブルを同期テーブルとして Lakebase にレプリケートしてください。
- **アプリからのトランザクション書き込み**（注文、セッション、監査ログ）。[Lakebase Postgres](/docs/lakebase/overview) を使ってください。
- **ガバナンスされたテーブルに対する自然言語での質問応答**。[Genie](/docs/agents/genie) を使ってください。
...
| やりたいこと                                                          | テンプレート                                                        |
| --------------------------------------------------------------------- | ------------------------------------------------------------------- |
| UC テーブルを Lakebase に複製して低レイテンシーで読み取る             | [Sync Tables (Autoscaling)](/templates/sync-tables-autoscaling)     |
| UC + Lakebase Change Data Feed + メダリオンパイプラインを一式構築する | [Operational Data Analytics](/templates/operational-data-analytics) |

## 次のステップ \{#where-to-next\}

- [分析的な読み取り](/docs/lakehouse/analytical-reads) — Analytics プラグイン、SQL ファイル、on-behalf-of-user クエリについて。
- [Lakeflow Jobs](/docs/lakehouse/jobs) — Jobs プラグイン、`runNow`、SSE による進捗通知について。
- [パイプラインとデータ鮮度](/docs/lakehouse/pipelines) — Analytics プラグインを利用した鮮度シグナルについて。