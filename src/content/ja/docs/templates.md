...
# テンプレートとは？ \{#what-are-templates\}

DevHubには、Databricksアプリのひな型を素早く作成できる[templates](/templates)のコレクションが用意されています。
...
1. [/templates](/templates) でテンプレートを開き、作りたいものに合ったものを選びます。
...
- [App with Lakebase](/templates/app-with-lakebase) — マネージド Postgres をバックエンドとする CRUD アプリ。
- [AI Chat App](/templates/ai-chat-app) — ストリーミング応答と永続的な会話履歴を備えたチャットアプリ。
- [Vacation Rentals Operations Console](/templates/vacation-rentals) — Lakebase によるフラグとエージェントノート、SQL warehouse による収益分析、埋め込み型 Genie チャットパネルを備えた予約キュー。
...
- [Onboard Your Coding Agent](/templates/onboard-your-coding-agent) — Databricksプラットフォームのスキルと Docs MCP Server をリポジトリにインストールします。
- [Lakebase Data Persistence](/templates/lakebase-data-persistence) — 既存のアプリにマネージドPostgresストレージを追加します。
- [Create a Lakebase Project](/templates/lakebase-create-instance) — Lakebaseプロジェクトをプロビジョニングし、接続情報を取得します。
...
- [templatesカタログ](/templates)の全体を見る。
- アプリ構築に活用できるDatabricksプラットフォームの各サービスをさらに詳しく見る: [Databricks Apps](/docs/apps/overview)、[Lakebase Postgres](/docs/lakebase/overview)、[Agent Bricks](/docs/agents/overview)、[Data Lakehouse](/docs/lakehouse/overview)。