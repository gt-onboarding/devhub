...
Databricks CLI (コマンドラインインターフェース) を使うと、ローカルのターミナルやコーディングエージェント経由で Databricks プラットフォームを操作できます。Databricks でアプリを開発する場合や、本サイトの[テンプレート](/ja/templates)を実行する場合は、CLI のインストールと認証が必要です。
...
* [エージェントスキルをインストール](/ja/docs/tools/ai-tools/agent-skills)して、AIコーディングアシスタントにDatabricksプラットフォームの知識を持たせます。
* [DevHub Docs MCP Server](/ja/docs/tools/ai-tools/docs-mcp-server)をセットアップして、AIコーディングアシスタントからDevHubのドキュメントにアクセスできるようにします。