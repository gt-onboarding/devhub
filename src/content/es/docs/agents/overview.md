...
Para saber qué es Agent Bricks y cómo desarrollar con él, consulta la [documentación de Agent Bricks](https://docs.databricks.com/aws/en/agents/agent-bricks/) o la agent skill [`databricks-agent-bricks`](/docs/tools/ai-tools/agent-skills).

Tu aplicación de AppKit se conecta a las capacidades de Agent Bricks mediante el [plugin de Model Serving](/docs/appkit/v0/plugins/model-serving), para agentes, modelos fundacionales y endpoints gobernados, y el [plugin de Genie](/docs/appkit/v0/plugins/genie), para consultas en lenguaje natural sobre tablas de Unity Catalog.

## Cómo encaja todo \{#how-it-fits-together\}

Tu aplicación de AppKit llama a Agent Bricks a través de un **endpoint de Model Serving** (un modelo fundacional, un Knowledge Assistant, un Supervisor Agent o un agente de Python personalizado) o de un **Genie Agent** (consultas en lenguaje natural sobre tablas de Unity Catalog). El [plugin de Model Serving](/docs/appkit/v0/plugins/model-serving) y el [plugin de Genie](/docs/appkit/v0/plugins/genie) cubren ambos casos.
...
- Las rutas se ejecutan como el usuario autenticado, por lo que los **permisos por usuario** se aplican automáticamente. Tus usuarios solo ven los endpoints y los datos a los que ya tienen acceso. Sin código de OAuth de tu parte. Consulta [Contexto de ejecución](/docs/appkit/v0/plugins/execution-context) para conocer los detalles.
...
| -------------------------------------------------------------- | ---------------------------------------------------------- |
| Añadir un chatbot con streaming a tu app                       | [AI Chat App](/templates/ai-chat-app)                      |
| Permitir que los usuarios consulten tablas en lenguaje natural | [Genie Analytics App](/templates/genie-analytics-app)      |
| Añadir el cambio entre varios agentes de Genie a una app existente | [Genie Multi-Agent Selector](/templates/genie-multi-space) |

## Qué sigue \{#where-to-next\}

- [Unity AI Gateway](/docs/agents/ai-gateway) para el acceso gobernado a modelos, endpoints de agentes y herramientas externas.
- [Genie Agents](/docs/agents/genie) para conversar con tus datos en tablas de Unity Catalog.
- [Endpoints de agentes personalizados](/docs/agents/custom-agents) para integrar Knowledge Assistant, Supervisor Agent o tu propio agente en Python.