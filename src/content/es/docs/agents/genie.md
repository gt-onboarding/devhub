...
Para probar un Genie Agent en el workspace antes de integrarlo, consulta [Usar un Genie Agent](https://docs.databricks.com/aws/en/genie-agents/talk-to-genie). Para crear o gestionar uno desde tu agente de programación, usa la agent skill [`databricks-genie-agents`](/docs/tools/ai-tools/agent-skills).
...
El [plugin `genie`](/docs/appkit/v0/plugins/genie) conecta todo eso con tu interfaz de chat y se encarga del streaming por SSE, la autenticación y la reproducción de conversaciones.
...
`status` alterna entre `idle`, `streaming`, `loading-history`, `loading-older` y `error`. Úsalo para gestionar los estados de carga en tu interfaz. El hook también devuelve `error`, `conversationId` y utilidades de paginación (`hasPreviousPage`, `isFetchingPreviousPage`, `fetchPreviousPage`). Consulta la [referencia del plugin Genie de AppKit](/docs/appkit/v0/plugins/genie) para ver el tipo de retorno completo y la [API de conversación de Genie](https://docs.databricks.com/aws/en/genie-agents/conversation-api) para la API REST subyacente.
...
Vincula cada ID a un recurso independiente en `app.yaml`. Consulta el template [Genie Multi-Agent Selector](/templates/genie-multi-space) para ver una interfaz funcional con cambio de agente, limpieza de conversaciones y sincronización de la URL.
...
Prueba la [Genie Analytics App](/templates/genie-analytics-app) para ver una configuración completa ya integrada, o explora los [endpoints de agentes personalizados](/docs/agents/custom-agents) para Knowledge Assistants y Supervisor Agents.