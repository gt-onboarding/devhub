...
El [Analytics plugin](/docs/appkit/v0/plugins/analytics) se encarga de las lecturas en el SQL warehouse (consulta [Lecturas analíticas](/docs/lakehouse/analytical-reads) y [Pipelines y actualidad de los datos](/docs/lakehouse/pipelines)). El [Jobs plugin](/docs/appkit/v0/plugins/jobs) se encarga de disparar las ejecuciones y seguir su progreso (consulta [Lakeflow Jobs](/docs/lakehouse/jobs)).
...
- **Lecturas de menos de un segundo en una solicitud del usuario**, como sugerencias mientras se escribe o autocompletado. Usa [Lakebase Postgres](/docs/lakebase/overview) directamente o replica una tabla de UC en Lakebase como tabla sincronizada.
- **Escrituras transaccionales desde tu aplicación** (pedidos, sesiones, registros de auditoría). Usa [Lakebase Postgres](/docs/lakebase/overview).
- **Preguntas y respuestas en lenguaje natural sobre tablas gobernadas**. Usa [Genie](/docs/agents/genie).
...
| Quieres...                                                        | Template                                                            |
| --------------------------------------------------------------------- | ------------------------------------------------------------------- |
| Replicar una tabla de UC en Lakebase para lecturas de baja latencia              | [Sync Tables (Autoscaling)](/templates/sync-tables-autoscaling)     |
| Montar el pipeline completo de UC + Change Data Feed de Lakebase + medallion | [Operational Data Analytics](/templates/operational-data-analytics) |

## Siguientes pasos \{#where-to-next\}

- [Lecturas analíticas](/docs/lakehouse/analytical-reads) con el Analytics plugin, archivos SQL y consultas on-behalf-of-user.
- [Lakeflow Jobs](/docs/lakehouse/jobs) para el Jobs plugin, `runNow` y el progreso vía SSE.
- [Pipelines y actualidad de los datos](/docs/lakehouse/pipelines) para obtener señales de actualidad mediante el Analytics plugin.